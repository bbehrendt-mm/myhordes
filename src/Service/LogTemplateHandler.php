...
class LogTemplateHandler
{
    ...
    public function bankItemTamerTakeLog( Citizen $citizen, ItemPrototype $item, bool $broken = false ): TownLogEntry {

        $variables = array('citizen' => $citizen->getId(), 'item' => $item->getId(), 'broken' => $broken, 'dogname' => $citizen->getId());
        $template = $this->entity_manager->getRepository(LogEntryTemplate::class)->findOneBy(['name' => 'bankTakeTamer']);
