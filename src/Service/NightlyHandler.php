...
class NightlyHandler
{
            ...
            if ($citizen->getAlive() && $citizen->getZone()) {
                ...
                if ($citizen_hidden) {
                    ...
                    if (!$this->random->chance($survival_chance)) {
                        ...
                        $this->grantBlueprintIfDeserved($citizen);
                    }
                }
                else {
    ...
    private function stage2_attack(Town &$town) {
        ...
        foreach ($watchers as $watcher) {
            ...
            $used_items = 0;
            foreach($ctz->getInventory()->getItems()->getValues() as $i){
                if($i->getPrototype()->getWatchpoint() > 0 || $i->getPrototype()->getName() === 'chkspk_#00'){
                    $used_items++;
                }
            }
     ...
     * @return void
     */
    public function grantBlueprintIfDeserved(mixed $citizen): void
    {
