...
class HordesFactFixtures extends Fixture implements DependentFixtureInterface {

    private $entityManager;

    protected static $facts_data = [
        // French Facts
    ...
    private function insertAwards(ObjectManager $manager, ConsoleOutputInterface $out) {
        $out->writeln('<comment>HordesFact: ' . count(static::$facts_data) . ' fixture entries available.</comment>');

        $progress = new ProgressBar( $out->section() );
        $progress->start( count(static::$facts_data) );

       foreach(static::$facts_data as $entry) {
