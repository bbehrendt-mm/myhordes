    ...
    public function newspaper(): Response {
        ...
        if (count($gazette_logs) == 0) {
            ...
            if ($day == 1) {
                ...
                if ($town->isOpen()){
                    $text .= "<p>" . $this->translator->trans("Die Stadt wird erst starten, wenn sie <strong>{$town->getPopulation()} Bürger</strong> hat.", [], 'game') . "</p>" . "<a class='help-button'>" . "<div class='tooltip help'>" . $this->translator->trans("Falls sich dieser Zustand auch um Mitternacht noch nicht geändert hat, findet kein Zombieangriff statt. Der Tag wird dann künstlich verlängert.", [], 'global') . "</div>" . $this->translator->trans("Hilfe", [], 'global') . "</a>";
                } else {
