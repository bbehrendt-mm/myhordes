...
class AdminUserController extends AdminActionController
{
                                         ...
                                         string $param = ''): Response
    {
        ...
        if (str_starts_with($action, 'dbg_') && (!$this->isGranted('ROLE_SUB_ADMIN') || ($kernel->getEnvironment() !== 'dev' && $kernel->getEnvironment() !== 'local')) )
            return AjaxResponse::error(ErrorHelper::ErrorInvalidRequest);
    ...
    public function user_give_picto(int $id, JSONRequestParser $parser, KernelInterface $kernel, EventProxyService $proxy): Response
    {
        ...
        if ($prototype_id === -42 && ($kernel->getEnvironment() === 'dev' || $kernel->getEnvironment() !== 'local') && $this->isGranted('ROLE_SUB_ADMIN', $user))
